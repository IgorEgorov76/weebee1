...
async function html1() {
    let text = (await (await fetch('/weebee1/activity.html')).text());
...
activity1.addEventListener('click', function (event) {
    ...
    clearInterval(textAll);
    history.pushState('', '', '/weebee1/activity.html');
    html1();
})

async function html2() {
    let text = (await (await fetch('/weebee1/map.html')).text());
    container.innerHTML = text;
}
...
map1.addEventListener('click', function (event) {
    event.preventDefault();
    clearInterval(textAll);
    history.pushState('', '', '/weebee1/map.html');
    async function go() {
       await html2();
       await init();
    }
...
async function html3() {
    let text = (await (await fetch('/weebee1/time.html')).text());
    container.innerHTML = text;
...
time1.addEventListener('click', function (event) {
    event.preventDefault();
    history.pushState('', '', '/weebee1/time.html')
    html3();
})
